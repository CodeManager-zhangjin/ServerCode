...
Qiniu_Error Qiniu_RS_List( Qiniu_Client* self, Qiniu_RS_ListRet* ret, const char* bucket,
						  const char* prefix, 
						  const char* marker, 
						  int limit, 
						  const char* delimiter )
{
	...
	strcat((char*)szTempBuf, (const char*)"bucket=");
	strcat((char*)szTempBuf, bucket);

	if (prefix && strlen(prefix))
	{
		strcat((char*)szTempBuf, (const char*)"&prefix=");
		strcat((char*)szTempBuf, prefix);
	}
	if (marker && strlen(marker))
	{
		strcat((char*)szTempBuf, (const char*)"&marker=");
		strcat((char*)szTempBuf, marker);
	}
	if (limit)
	{
		strcat((char*)szTempBuf, (const char*)"&limit=");
		char strlimit[16] = {0};
		snprintf(strlimit, 16, "%d", limit);
		strcat((char*)szTempBuf, (const char*)strlimit);
	}
	if (delimiter && strlen(delimiter))
	{
		strcat((char*)szTempBuf, (const char*)"&delimiter=");
		strcat((char*)szTempBuf, delimiter);
	}

	char* url = Qiniu_String_Concat3(QINIU_RSF_HOST, "/list?", szTempBuf);
	printf("Qiniu_RS_List url:%s\n", url);
	err = Qiniu_Client_Call(self, &root, url);
	Qiniu_Free(url);

	if (err.code != 200) return err;

	printf("Qiniu_RS_List ok\n");
	ret->marker = Qiniu_Json_GetString(root, "marker", 0);
	printf("Qiniu_RS_List marker:%s\n", ret->marker);

	Qiniu_Json* items = Qiniu_Json_FirstChild(root, "items", NULL);
	if (items == NULL) return err;
	ret->itemsize = cJSON_GetArraySize(items);
	printf("Qiniu_RS_List cJSON_GetArraySize %d\n", ret->itemsize);
	int i = 0;
	for (; i < ret->itemsize; i++)
	{
		Qiniu_Json* item = cJSON_GetArrayItem(items, i);
		ret->item[i].hash = Qiniu_Json_GetString(item, "hash", 0);
		ret->item[i].key = Qiniu_Json_GetString(item, "key", 0);
		ret->item[i].fsize = Qiniu_Json_GetInt64(item, "fsize", 0);
		ret->item[i].putTime = Qiniu_Json_GetInt64(item, "putTime", 0);
		printf("hash:%s\nkey:%s\nfsize:%lf\nputIme:%lf\n", ret->item[i].hash, ret->item[i].key, ret->item[i].fsize, ret->item[i].putTime);
	}
