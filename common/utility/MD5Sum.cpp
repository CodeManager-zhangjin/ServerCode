...
const char* MD5Sum::Calculate(const unsigned char* pachSource, uint32 nLen)
{
	MD5_CTX context;
	...
	memset (m_sHash, 0, sizeof m_sHash);
	for (int i = 0; i < 16; i++)
	{
		sprintf (m_sHash + 2 * i, "%02x", m_rawHash[i]);
	}

	return m_sHash;
}

const char* MD5Sum::GetHash() const
{
	return m_sHash;
}
