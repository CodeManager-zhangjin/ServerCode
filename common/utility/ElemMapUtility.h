...
// 管理类模板
template <class Elem>
class CElemMapUtility  
{
public:
	CElemMapUtility();
	virtual ~CElemMapUtility();
...
template <class Elem>
CElemMapUtility<Elem>::CElemMapUtility()
{
}
...
template <class Elem>
void CElemMapUtility<Elem>::ClearElem()
{
...
template <class Elem>
int CElemMapUtility<Elem>::AddElem(DWORD dwID, Elem* pElem)
{
	if (0 == dwID) return -1;
...
template <class Elem>
int CElemMapUtility<Elem>::DelElem(Elem* pElem)
{
	if (NULL == pElem) return -1;
	typename ELEM_MAP::iterator iter = m_mapElement.begin();
	for (; iter != m_mapElement.end(); iter++) {
...
template <class Elem>
int CElemMapUtility<Elem>::DelElem(DWORD dwID)
{
	if (0 == dwID) return -1;
...
template <class Elem>
Elem* CElemMapUtility<Elem>::GetElem(DWORD dwID)
{
