...
#define NETWORK_REASONPROXY_NEED_AUTH			15
#define NETWORK_REASONTCP_TRANS_ERROR			16
/*
 *	Option
 */
...
#define NETWORK_MEDIA_OPTION_REST_BUFFER			(NETWORK_MEDIA_OPTION_BASE+2)
/*
 *  Parameter pointer to ulong
 *	0 disable
 *  1 enable jitter buffer
 *	2 Normal
 *  3 N-Server Mode
 */
#define NETWORK_MEDIA_OPTION_JITTER_BUFFER_FLAG		(NETWORK_MEDIA_OPTION_BASE+3)
#define NETWORK_MEDIA_OPTION_CLEAR_LOCAL_BUFFER     (NETWORK_MEDIA_OPTION_BASE+4)
/*
 *	Network
 */
...
//Parameter is a pointer to ulong
//return value -1: not avaible, 0: Ok, 1 still evaluing, try later
#define NETWORK_NET_OPT_TYPE_GET_RTT_VALUE			(NETWORK_NETWORK_OPTION_GET_BASE+3)
...
struct ProxySetting 
{
	...
	char UserName[128]; /*the UserName[0] should be '\0' if no user name present*/
	char Password[128]; /*the Password[0] should be '\0' if no password present*/
};
...
public:
	...
	virtual int OnPeerIPChange(unsigned long dwPeerAddr, unsigned short wPort, INetConnection *pCon){return 0;}
...
public:
	...
	virtual int SendMediaWithTm(unsigned char *pHeader, int nHeaderLen, 
		char *pData, int nLen, unsigned char nMediaType, unsigned char nSubSeq, 
		unsigned short usIFrameSeq, unsigned long nSec, unsigned long nUSec)
	{
		return SendMedia(pHeader, nHeaderLen, pData, nLen, nMediaType, nSubSeq, usIFrameSeq);
	}
...
extern "C"
{
	...
	LIBNETWORK_EXPORT unsigned char *GetFCBuffer();
	LIBNETWORK_EXPORT void NetworkSetLevel(int nLevel);
}
