...
{
	LOG_DETAIL(LOG_DB_SERVER, "signal: %d received\n", nSignal);
	CServer::DeleteInstance();
}

void sig_int2( int nSignal )
{
	LOG_DETAIL(LOG_DB_SERVER, "signal: %d received 2\n", nSignal);
}

#define _VER_DB_		"1.0.0.7" 
...
int main( int argc, char* argv[] )
{
	...
	LOG_DETAIL(LOG_DB_SERVER, "The dvip server exit normally\n");
	CServer::DeleteInstance();
	return 0;
}
