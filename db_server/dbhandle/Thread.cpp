...
bool CThread::ThreadStart()
{
	...
	m_bRunning = true;
	LOG_DEBUG(LOG_MAIN, "ThreadStart Begin...ThreadID %lu\n", m_nThreadID);
	//////////////////////////////////////////////////////////////////////////
	sem_init(&m_sem, 0, 0);
	...
	ActivateThread();
	LOG_DEBUG(LOG_MAIN, "ThreadStart End! ThreadID %lu\n", m_nThreadID);
	return true;
}
...
bool CThread::ThreadStop()
{
	...
	m_bRunning = false;
	LOG_DEBUG(LOG_MAIN, "ThreadStop Begin...ThreadID %lu\n", m_nThreadID);
	ActivateThread();
	pthread_join(m_nThreadID, NULL);
	LOG_DEBUG(LOG_MAIN, "pthread_join end\n");
	sem_destroy(&m_sem);

	LOG_DEBUG(LOG_MAIN, "ThreadStop End! ThreadID %lu\n", m_nThreadID);
	return true;
}
